...
-- Generated by patch_skin_class_table.py from each weapon DA's cooked WeaponSkin soft ref.
-- Regenerate rather than hand-edit; an item missing here must never reach GetAsset, which
-- crashes on a package that does not exist. The three weapons absent from this table
-- (Beselbum, Milerim, Telarim) carry no skin reference in their DA at all.
local SKIN_CLASS = {
...
local function da_by_resident_scan(item)
    ...
    log("  FindAllOf returned ok=" .. tostring(ok) .. " n=" .. tostring(n))
...
local function apply_cine(cine, char, why)
    ...
    log("  DA for apply live=" .. tostring(is_live(da)) .. " " .. obj_label(da))
