...
local VIS_SHOWN = 4
local KSL_CDO = "/Script/Engine.Default__KismetSystemLibrary"
...
-- Shared/renamed skins. Do not add guessed paths.
local COOKED_SKIN = {
...
local PICTOS_DATA = "Pictos_Data_103_EE44D66B4E4F16A7FD44FF9F25777CF4"
...
local function pictos_from_static(static)
    if static == nil then
        return nil
    end
    local candidates = { pictos_prop, PICTOS_DATA, "Pictos_Data" }
    for _, n in ipairs(candidates) do
        if n then
            local v = safe_get(static, n)
            if is_live(v) then
                pictos_prop = n
                return v
            end
...
local function da_name_matches(full, item)
    if type(full) ~= "string" or not item then
        return false
    end
    ...
    local suffix = "_" .. item
    return asset:sub(-#suffix) == suffix
end

local function da_by_resident_scan(item)
    if type(FindAllOf) ~= "function" or not item then
        return nil
    end
    local ok, arr = pcall(FindAllOf, "BP_DataAsset_Weapon_C")
    if not ok or arr == nil then
        return nil
    end
    for i = 1, #arr do
        local da = arr[i]
        if is_live(da) and da_name_matches(safe_call(da, "GetFullName") or "", item) then
            return da
        end
    end
    return nil
...
local function cache_da(item, da, via)
    if not item or not is_live(da) then
        return false
    end
    local prev = da_by_item[item]
    da_by_item[item] = {
        path = object_path(da),
        cls_path = prev and prev.cls_path or nil,
    }
    log(
        "  DA cached item="
            .. item
...
local function remember_da(item, view, allow_scan)
    if not item then
        return
    end
    local rec = da_by_item[item]
    if rec ~= nil and rec.path and try_find(rec.path) then
        return
    end
    local static, dyn = item_static(view)
    local da = pictos_from_static(static) or pictos_from_static(dyn)
    if is_live(da) then
        cache_da(item, da, pictos_prop or "Pictos_Data")
        return
    end
    if not allow_scan then
        return
    end
    da = da_by_resident_scan(item)
    if is_live(da) then
        cache_da(item, da, "FindAllOf")
    end
end

local function da_for_item(item)
    local rec = da_by_item[item]
    if rec == nil or not rec.path then
        return nil
    end
    return try_find(rec.path)
end

local function describe_soft(v)
    if not DEBUG then
        return ""
    end
    if v == nil then
    ...
    local bits = { "type=" .. type(v), "tostring=" .. tostring(v), "live=" .. tostring(is_live(v)) }
    local ok_a, asset = pcall(function()
        return v.AssetPath
    end)
    if ok_a and asset ~= nil then
        local ok_p, pkg = pcall(function()
            return asset.PackageName:ToString()
        end)
        if ok_p and type(pkg) == "string" then
            bits[#bits + 1] = "PackageName=" .. pkg
        end
        local ok_n, an = pcall(function()
            return asset.AssetName:ToString()
        end)
        if ok_n and type(an) == "string" then
            bits[#bits + 1] = "AssetName=" .. an
        end
    end
    local ok_t, path = pcall(function()
        return v:ToString()
    end)
    if ok_t and type(path) == "string" and path ~= "" then
        bits[#bits + 1] = "ToString=" .. path
    end
    return table.concat(bits, " ")
end

local function looks_like_soft(v)
    if v == nil then
        return false
    end
    local ts = tostring(v)
    ...
    return ts:find("Soft", 1, true) ~= nil
end
...
local function guessed_skin_paths(da, item)
    local p = object_path(da) or ""
    local folder = p:match("/Weapons/([^/]+)/")
    if not folder then
        folder = p:match("DA_WeaponPictos_([^_]+)_")
    end
    if not folder or not item then
        return {}
    end
    local folders = { folder }
    if folder == "Verso" then
        folders = { "Noah", "Verso" }
    end
    local out = {}
    for i = 1, #folders do
        local f = folders[i]
        local asset = "BP_WeaponSkin_" .. f .. "_" .. item
        out[#out + 1] = "/Game/Characters/Weapons/" .. f .. "/" .. asset .. "." .. asset .. "_C"
    end
    return out
end
...
local function skin_from_da(da, item)
    local rec = da_by_item[item]
    local cached_path = (rec and rec.cls_path) or COOKED_SKIN[item]
    if cached_path then
        log("  cache lookup item=" .. tostring(item) .. " path=" .. cached_path)
        ...
        log("  cache path not resident, reloading")
        if rec then
            rec.cls_path = nil
        end
    end
    if not is_live(da) then
        return nil, "da-dead"
    end
    local soft = safe_get(da, "WeaponSkin")
    log("  WeaponSkin " .. describe_soft(soft))
    if not looks_like_soft(soft) then
        return nil, "WeaponSkin-not-soft"
    end
    local ksl = try_find(KSL_CDO)
    if not ksl then
        return nil, "no-KSL"
    end
    log("  about to LoadClassAsset_Blocking item=" .. tostring(item))
    local ok, ret = pcall(function()
        return ksl:LoadClassAsset_Blocking(soft)
    end)
    if not ok then
        log("  LoadClassAsset_Blocking threw: " .. tostring(ret))
        return nil, "load-threw"
    end
    log("  LoadClassAsset_Blocking returned " .. tostring(ret))
    local cls = as_class(ret)
    if cls then
        cache_class(item, cls, "resolved")
        return cls, "resolved"
    end
    local paths = guessed_skin_paths(da, item)
    local cooked = COOKED_SKIN[item]
    if cooked then
        paths[#paths + 1] = cooked
    end
    for i = 1, #paths do
        local found = try_find_class(paths[i])
        if found then
            cache_class(item, found, "presence")
            return found, "presence " .. paths[i]
        end
    end
    local load_paths = {}
    if cooked then
        load_paths[1] = cooked
    elseif type(item) == "string" and item:match("_%d+$") then
        for i = 1, #paths do
            load_paths[#load_paths + 1] = paths[i]
        end
    end
    for i = 1, #load_paths do
        local found = load_guessed_class(load_paths[i])
        if found then
            cache_class(item, found, "getasset")
            return found, "getasset " .. load_paths[i]
        end
    end
    return nil, "unresolved-return"
end
...
local function apply_cine(cine, char, why)
    ...
    local da = da_for_item(item)
    log("  DA for apply: " .. obj_label(da))
    if not is_live(da) then
        log("  apply FAIL DA not live item=" .. tostring(item))
        apply_ok[char] = false
        return false
    end
    local cls, how = skin_from_da(da, item)
...
local function spawn_glam_weapon(pawn, cls)
    ...
    log("  about to BeginDeferredActorSpawnFromClass (if the log stops here, this call native-AVd)")
    ...
    log("  about to FinishSpawningActor")
...
local function apply_battle(pawn, char, why)
    ...
    local da = da_for_item(item)
    log("  DA for battle: " .. obj_label(da))
    if not is_live(da) then
        log("  battle apply FAIL DA not live item=" .. tostring(item))
        return false
    end
    local cls, how = skin_from_da(da, item)
    log(
    ...
    log("  about to RegisterExternalActor")
    ...
    log("  about to Attach Weapons to Hands")
    ...
    if is_live(old) and not classes_equal(old, spawned) then
        log("  about to K2_DestroyActor old Weapons")
        pcall(function()
            old:K2_DestroyActor()
        end)
    end
...
local function on_hide_tooltip(ctx)
    ...
    log("  HardcodedNameID=" .. tostring(char) .. " last.item=" .. tostring(last.item))
    ...
    if glam[char] == last.item then
        ...
        remember_da(last.item, last.view, true)
